...
const AboutSection = () => {
  ...
  return (
    <motion.div
      className="h-full w-screen flex flex-col items-center text-center justify-center px-4"
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, ease: "easeOut" }}
      viewport={{ once: false, amount: 0.3 }}
    >
      <BubbleText text="ABOUT ME" style="text-8xl font-bold opacity-50 mb-8 tracking-tighter"/>
      <div className="flex justify-center gap-0 mb-8 relative w-full max-w-5xl shadow-xl">
        ...
        <div className="relative">
          <img
            src="/luigi.jpeg"
            alt="luigi's photo"
            className="w-[280px] h-auto object-cover aspect-square rounded-bl-2xl rounded-tl-2xl z-0 opacity-85"
          />
          <div className="absolute top-0 right-0 h-full w-1/2 bg-gradient-to-r from-transparent to-indigo-300 z-10" />
        </div>

        {/* Right: Content Box */}
        <div className="relative flex flex-col items-center z-10 bg-gradient-to-r from-indigo-300 to-indigo-900 rounded-r-2xl p-6 flex-1 overflow-hidden">
